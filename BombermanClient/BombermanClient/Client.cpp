...
void Client::disconnect(Player* p) //Disconnect the player from the server
{
	sf::Packet temp;
	temp << 1;
	temp << p->getID();
...
void Client::sendPosition(Player* p)
{
	if (m_connected)
	{
		sf::Packet temp;
...
void Client::sendSpBomb(Player* p)
{
	sf::Packet temp;
	temp << 8;
	temp << p->getSpBombPosX();
	temp << p->getSpBombPosY();
	if (connection.send(temp) != sf::Socket::Done)
	{
		std::cout << "Error sending special bomb" << std::endl;
	}
}

void Client::receive(std::vector<std::unique_ptr<Enemy>>& enemies, Player* p)
{
